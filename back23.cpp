...
Back23::Back23() {
	texture.loadFromFile(IMAGES_FOLDER + BACK12_IMAGES_FOLDER + back23_anim_files[0]);
...
void Back23::update() {
	...
	if (now.asMilliseconds() > frame_delay) {
		...
		if (frame < 6) {
			...
			texture.loadFromFile(IMAGES_FOLDER + BACK12_IMAGES_FOLDER + back23_anim_files[frame]);
			sprite.setTexture(texture);
		}
		else {
			del = true;
